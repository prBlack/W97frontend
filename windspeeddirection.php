...
<div class='avgw'><img src="css/windicons/avgw.svg" width='24px' height='24px'></img></div>
<grey> Avg </grey><?php echo $weather["wind_direction_avg"],'&deg;'?> <?php echo $weather["wind_speed_avg"],"  <grey>",$weather["wind_units"], "</grey>";?>
</div></div>
<div class="windrun">
<?php 
if($weather["wind_run"] && $weather["wind_units"]=='mph'){echo '<windrun> Wind Run: </windrun><span>',$weather["wind_run"],' mi</span>','  ';}
else if($weather["wind_run"] && $weather["wind_units"]=='km/h'){echo '<windrun> Wind Run: </windrun><span>',$weather["wind_run"],' km</span>','  ';}
else if($weather["wind_run"] && $weather["wind_units"]=='m/s'){echo '<windrun> Wind Run: </windrun><span>',number_format($weather["wind_run"],1),' km</span>','  ';}?>
</windrun>
</div>
<br />
...
<?php echo "<span>Max </span>"."<max><b>".number_format($weather["wind_speed_max"],1)."</max></b>"."<supmb> ".$weather["wind_units"]."</supmb> Wind (".$weather["maxwindtime"].")";?></div>
<div class="gustspeedtrend1">
<?php echo "<span> Max </span>"."<max><b>".number_format($weather["wind_gust_speed_max"],1)."</max></b>"."<supmb> ".$weather["wind_units"]."</supmb> &nbsp;Gust (".$weather["maxgusttime"].")";?></div>
