	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM"),y:parseFloat(rowData[14]*<?php echo $windconv;?>)});
		...
		title: {
            text: "<?php echo $stationName;?> <?php echo $lang['Windspeed'] ;?> - <?php echo $lang['Gust'] ;?> (<?php echo $windunit ;?>) <?php echo date('Y') ;?>",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
