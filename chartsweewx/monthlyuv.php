<?php
	...
	include('/var/www/html/settings.php');	include('conversion.php');include('/var/ww/html/common.php');include('../settings1.php');
    echo '
	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0],'YYYY-MM-DD').format("ddd Do"),y:parseFloat(rowData[18])});
			}
		}
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0],'YYYY-MM-DD').format("ddd Do"),y:parseFloat(rowData[18])});
		...
		title: {
            text: "<?php echo $stationName;?> Ultra Violet Index <?php echo strftime(" %B") ;?> <?php echo date(" Y") ;?>",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
