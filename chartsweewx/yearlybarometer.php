<?php
	...
	include('/var/www/html/settings.php');include('conversion.php');include('/var/www/html/common.php');include('../settings1.php');	
    echo '
...
<html>
	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM"),y:parseFloat(rowData[10]*<?php echo $pressureconv ;?>)});
			}
		}
	...
	function processData2(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM"),y:parseFloat(rowData[11]*<?php echo $pressureconv ;?>)});
	...
	function drawChart( dataPoints1 , dataPoints2 ) {
		...
		title: {
            text: "<?php echo $stationName;?> <?php echo $lang['Barometer'] ;?> (<?php echo $pressureunit ;?>) <?php echo date('Y') ;?>",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
		...
		axisY:{
		title: "Barometer (<?php echo $pressureunit ;?>)",
		titleFontColor: "#ccc",
		titleFontSize: 10,
        titleWrap: false,
		margin: 10,
