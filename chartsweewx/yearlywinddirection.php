...
<html>
	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM"),y:parseFloat(rowData[16])});
			}
		}
	...
	function processData2(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM"),y:parseFloat(rowData[16])});
	...
	function drawChart( dataPoints1 , dataPoints2 ) {
		...
		title: {
            text: "<?php echo $stationName;?> Wind Direction (°) <?php echo date(" Y") ;?>",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
        ...
        return e.value .toFixed(2) + " <?php echo ° ;?> " ;  
         },		 
