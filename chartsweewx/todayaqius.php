...
<html>
	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0]).format('HH:mm'),y:parseFloat(rowData[15])});
			}
		}
		...
		if(allLinesArray.length>0){
			...
			for (var i = 1; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0]).format('HH:mm'),y:parseFloat(rowData[15])});
	...
	function drawChart( dataPoints1 , dataPoints2 ) {
		...
		title: {
            text: "<?php echo $stationName;?> Air Quality Today",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
