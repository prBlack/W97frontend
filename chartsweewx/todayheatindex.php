...
<html>
	<head>
		...
		<title>HEAT INDEX DATABASE CHART</title>	
	...
	function processData2(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[15]).format('HH:mm'),y:<?php echo $dewpointconv ;?>)});
		...
		title: {
            text: " <?php echo $stationName ;?> <?php echo $lang['Temperature'] ;?> - <?php echo $lang['Heat Index'] ;?> (<?php echo $tempunit ;?>) <?php echo $lang['Today'];?>",
			fontSize: 11,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
		...
		axisY:{
		title: "<?php echo $lang['Temperature'] ;?> - Heat Index ;?> (<?php echo $tempunit ;?>) ",
		titleFontColor: "#ccc",
		titleFontSize: 10,
        titleWrap: false,
		margin: 10,
