...
<html>
	<head>
		...
		<title>MONTH HUMIDITY DATABASE CHART</title>	
	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0]).format('ddd Do'),y:<?php echo $temperatureconv ;?>)});
			}
		}
	...
	function processData2(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0]).format('ddd Do'),y:parseFloat(rowData[8])});
		...
		title: {
            text: "<?php echo $stationName ;?> <?php echo $lang['Temperature'] ;?> (<?php echo $tempunit ;?>) - <?php echo $lang['Humidity'] ;?> (<?php echo "%" ;?>) <?php echo strftime(" %B") ;?> <?php echo date(" Y") ;?>",			fontSize: 11,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
		toolTip:{
