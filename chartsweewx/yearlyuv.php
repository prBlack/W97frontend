	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM:mm"),y:parseFloat(rowData[17])});
			}
		}
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM:mm"),y:parseFloat(rowData[17])});
		...
		title: {
            text: "<?php echo $stationName;?> Ultra Violet Index <?php echo date(" Y") ;?>",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
