	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0]).format('MMM'),y:parseFloat(rowData[18])});
			}
		}
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0]).format('MMM'),y:parseFloat(rowData[18])});
		...
		title: {
            text: "<?php echo $stationName;?> <?php echo $lang['Solarradiation'] ;?> (w/m2)  <?php echo date(" Y") ;?>",
			fontSize: 12,
			fontColor:' #ccc',
			fontFamily: "arial",
        },
