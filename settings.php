...
include('version.php');
	####################################################################################################
	#	HOME WEATHER STATION FIXED SETUP    								                           #
	#	CREATED FOR HOMEWEATHERSTATION TEMPLATE at https://weather34.com/homeweatherstation/index.html # 
	# 	   FILE= SETTINGS.PHP                                                                          #
	# 	                                                                                               #
	# 	DASHBOARD CU-X7    MAR 2018	   		       						                               #
...
//$showDate           = false; // whether to show date instead of just "Updated" on each panel
...
$trendSource        = 'file'; // source of weather trend data; only supports a cached data file at present
$trendFile          =  $livedata . '-trend'; // file to use for simulating trend data
$dataSource         = 'wunderground'; // 'wunderground' = weather underground server, only change if you are using an additional source for chart data such as a local mysql database 
...
$templateversion    = 'Dashboard CU(24-08-2018)';
$software    = 'Cumulus <span>Software</span>';$designedfor= '<br>For Cumulus';$chartversion = '(WeeWXDATACHARTSversion 1)';$somethinggoeshere ='d4586dec-e7a2-47ae-99b6-25527b2563c9';$chartversionmysql  =  '(DATACHARTS version 15 Mysqli)';
 //set the theme color
...
$theme = $_COOKIE['theme'];}
//set the units 
$units = ""; if (array_key_exists('units', $_COOKIE)) { $units = $_COOKIE['units']; } if (array_key_exists('units', $_GET) && ($_GET['units'] == 'us' || $_GET['units'] == 'uk' || $_GET['units'] == 'metric' || $_GET['units'] == 'scandinavia')) { setcookie('units', $_GET['units']); $units = $_GET['units']; } if ($units == 'uk') { $windunit = 'mph'; $tempunit = 'C'; $rainunit = 'mm'; $pressureunit = "hPa"; } else if ($units == 'scandinavia') { $windunit = 'm/s'; $tempunit = 'C'; $rainunit = 'mm'; $pressureunit = "hPa"; } else if ($units == 'metric') { $windunit = 'km/h'; $tempunit = 'C'; $rainunit = 'mm'; $pressureunit = "hPa"; } else if ($units == 'us') { $windunit = 'mph'; $tempunit = 'F'; $rainunit = 'in'; $pressureunit = "inHg"; } 
...
//function strftime_rus($format, $date = FALSE) {
	    ...
	    switch ($month_number) {
	        ...
	        case 3: $rus = 'МАРТ'; break;
	        case 4: $rus = 'АПР'; break;
	        case 5: $rus = 'МАЙ'; break;
	        case 6: $rus = 'ИЮНЬ'; break;
	        case 7: $rus = 'ИЮЛЬ'; break;
